...
### 手写动态代理和jdk动态代理的区别
  1. 验证性无jdk动态代理全面
  2. jdk动态代理没有产生java文件, 没有IO操作, 性能高
  3. jdk是通过byte数组(**代理类的内容**)直接编译成Class对象(**使用native原生类实现**)
  4. 都是通过反射来编写增强类的内容

### asm
  1. `ASM`是一个操作字节码的框架, `cglib`动态代理就是基于它实现的

### javassist
  1. 也是一个操作字节码的框架, hibarnate就是基于它来实现的


### cglib和jdk代理的效率
  1. 底层都是操作字节码文件, 效率没有很大区别
  2. 使用场景的话看需求, 看要基于接口来代理还是基于父类来代理  